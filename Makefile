...
lint-py:  ## run python linter with ruff
	...
	python -m ruff format --check af1
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix af1
	python -m ruff format af1
...
coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v af1/tests --cov=af1 --cov-report term-missing --cov-report xml
